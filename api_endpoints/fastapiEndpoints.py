...
from agents.route_agent import router_agent
...
async def send_message(message: FrontendSendMessage):
    ...
    messages_history[message.uuid].add_message(
        Message(sender=Sender.USER, text=message.text)
    )

    start_time = time.time()

    # ---------------------------------------------------------
    # 1️⃣ Route Agent decides what to do
    # ---------------------------------------------------------
    route_result = router_agent(
        settings,
        logger,
    ...
    # ---------------------------------------------------------
    # 2️⃣ Act based on router decision
    # ---------------------------------------------------------
    if route_result["action"] == "respond":
        reply = route_result["final_answer"]
        response_source = "route_agent"

    elif route_result["action"] == "call_db_agent":
        reply = db_agent(
            settings,
            logger,
            db_conn_str,
        ...
        response_source = "db_agent"

    else:
        logger.error(f"Unknown route action: {route_result}")
        reply = "Sorry, something went wrong while routing your request."
        response_source = "error"

    elapsed_time = time.time() - start_time
    logger.info(
        f"Bot response execution took {elapsed_time:.2f} seconds "
        f"(source={response_source})"
    )

    # ---------------------------------------------------------
    # 3️⃣ Sync Logic: Clear Local History if Agent Reset Memory
    # ---------------------------------------------------------
    if "Memory has been reset successfully" in reply:
        ...
        messages_history[message.uuid] = MessagesList()
        messages_history[message.uuid].add_message(
            Message(sender=Sender.ASSISTANT, text=reply)
        )
    else:
        messages_history[message.uuid].add_message(
            Message(sender=Sender.ASSISTANT, text=reply)
